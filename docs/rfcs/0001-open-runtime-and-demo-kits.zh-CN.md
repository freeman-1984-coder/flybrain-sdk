...
本文保留研究时的方案和接口草案，使用方式以当前 API 文档为准。`v0.2.0a1` 已实现真实子回路和开放 Python 接口；`v0.3.0a1` 增加 JS CPU 实验室、可编辑 HTML demo 和 Python 回放。`v0.4.0a1` 已增加通用会话、游戏反馈记录和音频生成；`v0.4.0a2` 增加可编辑项目生成器。实时游戏、音频输入、学习和经过硬件验收的 CUDA 仍待完成。后续原帖核查和实现决定见[补充调查](../research-2026-09-10.zh-CN.md)。下文按原始设计保留历史状态，不能作为当前功能清单。
