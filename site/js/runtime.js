...
function integer(x, label, min = 1) {
...
export class FlyBrain {
    ...
    static load(data, options = {}) {
        if (options.backend && options.backend !== "cpu")
            throw new Error(`${options.backend} is not implemented`);
        if ("format" in data) {
            if (data.format !== "flybrain-model-bundle" || data.schema_version !== 1)
                throw new Error("Unsupported model bundle");
            return new FlyBrain(data.model, { ...data.config, ...options.config });
        }
        return new FlyBrain(data, options.config);
    }
    constructor(model, config = {}) {
        if (model.schema_version !== 1 || typeof model.name !== "string" || !model.name)
            throw new Error("Invalid connectome");
        if (!Array.isArray(model.neuron_ids) || !model.neuron_ids.length ||
            model.neuron_ids.some(n => typeof n !== "string" || !n) ||
            new Set(model.neuron_ids).size !== model.neuron_ids.length)
            throw new Error("Neuron IDs must be unique strings");
        ...
        for (const [k, v] of Object.entries(cfg))
            finite(v, k);
        for (const k of ["dt_ms", "tau_ms", "input_gain", "rate_tau_ms", "action_rate_hz"]) {
            if (cfg[k] <= 0)
                throw new Error(`${k} must be positive`);
        }
        if (cfg.refractory_ms < 0 || cfg.reset >= cfg.threshold || cfg.rest >= cfg.threshold)
            throw new Error("Invalid LIF bounds");
        ...
        if (!Array.isArray(model.synapses))
            throw new Error("Missing synapses");
        for (const edge of model.synapses) {
            this.resolve([edge.pre]);
            this.resolve([edge.post]);
            finite(edge.weight, "weight");
        }
        for (const ports of [model.sensory, model.motor]) {
            record(ports, "ports");
            for (const [name, ids] of Object.entries(ports)) {
                if (!name)
                    throw new Error("Empty port name");
                this.resolve(ids);
            }
        }
        ...
        for (const [id, values] of Object.entries(model.annotations ?? {})) {
            this.resolve([id]);
            record(values, "annotation");
            if (Object.values(values).some(v => typeof v !== "string"))
                throw new Error("Annotations must be strings");
        }
    ...
    get timeMs() { return this.ticks * this.cfg.dt_ms; }
    get model() { return clone(this.graph); }
    ...
    resolve(ids) {
        if (!Array.isArray(ids) || !ids.length || ids.some(n => typeof n !== "string" || !this.index.has(n)))
            throw new Error("Expected known string neuron IDs");
        if (new Set(ids).size !== ids.length)
            throw new Error("Duplicate neuron IDs");
    ...
    select(attributes = {}) {
        ...
        for (const [k, v] of Object.entries(attributes)) {
            if (!available.has(k) || typeof v !== "string")
                throw new Error(`Unknown annotation or invalid selector: ${k}`);
        }
        const ids = this.graph.neuron_ids.filter(n => Object.entries(attributes).every(([k, v]) => this.graph.annotations?.[n]?.[k] === v));
        if (!ids.length)
            throw new Error("Selector matches no neurons");
        return ids;
    }
    stimulate(input) {
        const { channel, strength = 1, durationMs = 20 } = input;
        if (!Object.hasOwn(this.graph.sensory, channel))
            throw new Error(`Unknown sensory channel ${channel}`);
        finite(strength, "strength");
        finite(durationMs, "durationMs");
        if (strength < 0 || strength > 1 || durationMs <= 0)
            throw new Error("Invalid stimulus bounds");
    ...
    inject(ids, options) {
        this.resolve(ids);
        finite(options.amplitude, "amplitude");
        const steps = finite(options.durationMs, "durationMs") / this.cfg.dt_ms;
        if (Math.abs(steps - Math.round(steps)) > 1e-9)
            throw new Error("Duration must align with dt");
        this.pending.push({ ids: [...ids], amplitude: options.amplitude, remaining_steps: integer(Math.round(steps), "duration steps") });
        return this;
    }
    clearStimuli() { this.pending = []; }
    silence(ids, enabled = true) {
        if (typeof enabled !== "boolean")
            throw new Error("enabled must be boolean");
        for (const i of this.resolve(ids))
            this.silenced[i] = Number(enabled);
    }
    bindReadout(channels, scaleHz = this.cfg.action_rate_hz) {
        record(channels, "readout");
        finite(scaleHz, "scaleHz");
        if (scaleHz <= 0)
            throw new Error("scaleHz must be positive");
    ...
    advance(steps) {
        integer(steps, "steps");
        if (!Number.isSafeInteger(this.tick + steps))
            throw new Error("Tick overflow");
        for (let k = 0; k < steps; k++)
            this.oneTick();
    }
    step(steps = 1) { this.advance(steps); return this.state; }
    oneTick() {
        const n = this.index.size;
        const synaptic = new Float64Array(n), current = new Float64Array(n);
        for (let e = 0; e < this.weights.length; e++)
            synaptic[this.post[e]] += this.weights[e] * this.spikes[this.pre[e]];
        for (const pulse of this.pending)
            for (const id of pulse.ids)
                current[this.index.get(id)] += pulse.amplitude;
    ...
    static restore(data) {
        if (data.format !== "flybrain-js-checkpoint" || data.schema_version !== 1 || data.dynamics_revision !== "lif-exact-v1")
            throw new Error("Unsupported JS checkpoint");
        const brain = FlyBrain.load(data.model, { config: data.config });
        const n = brain.index.size, s = data.state;
        integer(s.tick, "tick", 0);
        if (s.time_ms !== s.tick * brain.cfg.dt_ms)
            throw new Error("Invalid checkpoint time");
        for (const name of ["voltage", "spikes", "rates_hz", "refractory", "silenced"]) {
            if (!Array.isArray(s[name]) || s[name].length !== n)
                throw new Error(`Invalid ${name} shape`);
        }
        for (let i = 0; i < n; i++) {
            if (finite(s.voltage[i], "voltage") >= brain.cfg.threshold)
                throw new Error("Invalid voltage");
            const rate = finite(s.rates_hz[i], "rate");
            if (rate < 0 || rate > 1000 / brain.cfg.dt_ms)
                throw new Error("Invalid rate");
            if (typeof s.spikes[i] !== "boolean" || typeof s.silenced[i] !== "boolean")
                throw new Error("Expected boolean state");
            if (integer(s.refractory[i], "refractory", 0) > brain.hold)
                throw new Error("Invalid refractory counter");
        }
        if (!Array.isArray(data.pending_currents))
            throw new Error("Invalid pending currents");
        for (const p of data.pending_currents) {
            brain.resolve(p.ids);
            finite(p.amplitude, "amplitude");
            integer(p.remaining_steps, "remaining_steps");
        }
        brain.bindReadout(data.readout.channels, data.readout.scale_hz);
        brain.ticks = s.tick;
        brain.v.set(s.voltage);
        brain.spikes.set(s.spikes.map(Number));
        brain.rates.set(s.rates_hz);
        brain.refractory.set(s.refractory);
        brain.silenced.set(s.silenced.map(Number));
        brain.pending = clone(data.pending_currents);
        return brain;
    }
}
