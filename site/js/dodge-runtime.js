...
function bounded(x, a, b) {
    if (typeof x !== "number" || !Number.isFinite(x) || x < a || x > b)
        throw new Error(`Expected number in ${a}..${b}`);
    return x;
}
function integer(x, max = Number.MAX_SAFE_INTEGER) {
    bounded(x, 0, max);
...
function arena(data) {
    if (data.type !== "dodge-arena-v1")
        throw new Error("Unsupported arena");
    integer(data.seed, 2 ** 32 - 1);
    integer(data.rng, 2 ** 32 - 1);
    integer(data.frame);
    bounded(data.x, 0, 1);
    integer(data.collisions);
    integer(data.passed);
    if (!Array.isArray(data.blocks) || data.blocks.length > 64)
        throw new Error("At most 64 obstacles");
...
export class DodgeSession {
    ...
    events = [];
    frames = [];
    constructor(bundle, seed = 7) {
        integer(seed, 2 ** 32 - 1);
        if (bundle.model.neuron_ids.length > 1000 || bundle.model.synapses.length > 100000)
            throw new Error("Live demo supports small circuits only");
        this.modelName = bundle.model.name;
        this.neuronCount = bundle.model.neuron_ids.length;
        this.brain = FlyBrain.load(bundle);
        // This adapter uses the public demo's exact period; other periods use Python Session.
        if (this.brain.config.dt_ms !== 1)
            throw new Error("Dodge adapter requires dt_ms=1");
        this.modelHash = bundle.model_sha256;
        if (bundle.model.name === "toy-v1")
            this.outputs = ["motor.turn_right", "motor.turn_left"];
        else if (this.modelHash === "47a0c91b3eba9c606c29faef58fe15c8846fc049c4314a5e3b6912ab22c5e68a")
            this.outputs = ["10010", "10001"];
        else
            throw new Error("Dodge preset requires toy-v1 or the pinned MaleCNS escape circuit");
    ...
    get frameIndex() { return this.world.frame; }
    get environment() { return copy(this.world); }
    ...
    get outputIds() { return [...this.outputs]; }
    get outputRates() { return this.brain.observe(this.outputs).rates_hz; }
    get recordedFrames() { return this.frames.length; }
    observe() {
        let left = 0, right = 0;
    ...
    command(command) {
        ...
        if (command.op === "add") {
            bounded(command.x, 0, 1);
            bounded(command.y, 0, 0.95);
            if (this.world.blocks.length >= 64)
                throw new Error("At most 64 obstacles");
            ...
            bounded(command.input_gain, 0, 4);
            bounded(command.output_gain, 0, 2);
            this.gain = command.input_gain;
            this.outputGain = command.output_gain;
        }
        else if (command.op === "silence") {
            if (typeof command.enabled !== "boolean")
                throw new Error("Silence requires boolean");
            this.brain.silence(this.outputs, command.enabled);
            this.muted = command.enabled;
        }
        else
    ...
    step() {
        if (this.frames.length >= 5000)
            throw new Error("100-second recording limit reached. Export and reset.");
        if (this.brain.tick !== this.frameIndex * 20)
            throw new Error("Neural clock mismatch");
        const observation = this.observe();
        [observation.danger_left, observation.danger_right].forEach((value, i) => {
            if (value * this.gain)
                this.brain.inject(this.inputs[i], { amplitude: value * this.gain, durationMs: 20 });
        });
        ...
        for (const block of this.world.blocks) {
            ...
            if (y >= 1) {
                if (Math.abs(block.x - this.world.x) < 0.13)
                    this.world.collisions++;
                else
                    this.world.passed++;
            }
            else
                remaining.push({ x: block.x, y });
        }
        this.world.blocks = remaining;
        this.world.frame++;
        if (this.frameIndex % 45 === 0 && this.world.blocks.length < 64)
            this.spawn();
        const frame = { index: this.frameIndex, elapsed_ms: this.frameIndex * 20, brain_tick: this.brain.tick,
            observation, rates_hz: rates, requested: { steer }, applied: { steer: (this.world.x - previous) / 0.02 }, environment: this.environment };
        this.frames.push(copy(frame));
        return frame;
    }
    save() {
    ...
    static restore(data) {
        if (data.format !== "flybrain-dodge-checkpoint" || data.schema_version !== 1)
            throw new Error("Unsupported dodge checkpoint");
        const world = arena(data.environment);
        bounded(data.input_gain, 0, 4);
        bounded(data.output_gain, 0, 2);
        if (typeof data.silenced !== "boolean")
            throw new Error("Invalid silence state");
        const brain = FlyBrain.restore(data.brain);
        if (brain.tick !== world.frame * 20)
            throw new Error("Checkpoint clocks differ");
        ...
        if (outputIndices.some(i => data.brain.state.silenced[i] !== data.silenced))
            throw new Error("Silence state differs");
        result.brain = brain;
        result.world = world;
        result.gain = data.input_gain;
        result.outputGain = data.output_gain;
        result.muted = data.silenced;
        result.initial = result.save();
        return result;
    }
    static replay(data) {
        if (data.format !== "flybrain-dodge-recording" || data.schema_version !== 1 ||
            !Array.isArray(data.frames) || data.frames.length > 5000 ||
            !Array.isArray(data.events) || data.events.length > 10000)
        ...
        for (const frame of data.frames) {
            events();
            const actual = result.step();
            if (actual.index !== frame.index || actual.brain_tick !== frame.brain_tick || actual.elapsed_ms !== frame.elapsed_ms)
                throw new Error("Recording clock mismatch");
            compare(actual, frame);
        }
        events();
        if (cursor !== data.events.length)
            throw new Error("Unordered or out-of-range events");
        if (result.frameIndex !== data.final.environment.frame || result.brain.tick !== data.final.brain.state.tick)
            throw new Error("Final clock mismatch");
        compare(result.save(), data.final);
        return result;
    }
}
function compare(a, b) {
    if (typeof a === "number" && typeof b === "number" && Number.isFinite(b) && Math.abs(a - b) <= 1e-9)
        return;
    if (a === b)
        return;
    if (a && b && typeof a === "object" && typeof b === "object" && Array.isArray(a) === Array.isArray(b)) {
        ...
        if (keys.length === Object.keys(bb).length && keys.every(k => Object.hasOwn(bb, k))) {
            keys.forEach(k => compare(aa[k], bb[k]));
            return;
        }
    }
